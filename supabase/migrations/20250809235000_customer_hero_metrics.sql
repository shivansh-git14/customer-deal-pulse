-- ==========================================
-- CUSTOMER HERO METRICS FUNCTION
-- ==========================================
-- Purpose: Calculate customer health and engagement metrics for hero cards
-- Last Updated: 2025-08-09
-- Strategy: Secure PostgreSQL function following consolidation guidelines
-- Dependencies: customers, customer_stage_historical, contacts, revenue
-- ==========================================
...
-- ==========================================
-- Purpose: Calculate customer health metrics (at-risk rate, decision makers, health score, repeat revenue)
-- Parameters: 
...
-- Returns: TABLE with at_risk_rate, customers_with_dm_rate, health_engagement_score, repeat_revenue_rate
-- Last Updated: 2025-08-09
...
BEGIN
    ...
    customer_universe AS (
        ...
        -- Apply manager filter through revenue or customer relationships
        AND (
            p_manager_id IS NULL 
            OR EXISTS (
                SELECT 1 FROM revenue r 
                INNER JOIN filtered_reps fr ON r.sales_rep = fr.sales_rep_id
    ...
    revenue_analysis AS (
        ...
        SELECT 
            COALESCE(
                SUM(CASE WHEN revenue_dates > 1 THEN total_revenue ELSE 0 END) / 
